...
explain analyze -- anti join
 select a.m_id, b.m_id
   from member a
   left join member b on b.m_id = a.m_id
  where b.m_area is null;

explain analyze
select a.m_id, b.m_id
  from member a, member b
 where b.m_id in (1, 2, 3, 4); -- left join 사용


explain
select * from member a, member b where a.m_id = b.m_id; -- cross join 사용
...
select *
  ...
  join lateral ( select *
                  ...
                  order by m_id desc
                  limit 10
      ) b on a.m_id = b.m_id; -- lateral join 사용
